...
class Test3Dataset(BaseDataset):
    ...
    def __init__(self, opt,phase):
        ...
        path_tif_high_dose = sorted(glob(os.path.join(opt.dataroot_valid, 'HD_TE')))
        path_tif_low_dose = sorted(glob(os.path.join(opt.dataroot_valid, opt.test_input)))
        self.path_high_dose_train = []
        self.path_low_dose_train = []
        
        for p in path_tif_high_dose:
            tmp_path_full_dose = glob(p + '/*.png')            
            self.path_high_dose_train.extend(tmp_path_full_dose)
        for p in path_tif_low_dose:     
            tmp_path_low_dose = glob(p + '/*.png')
            self.path_low_dose_train.extend(tmp_path_low_dose) 
        self.path_low_dose_train = sorted(self.path_low_dose_train)
        self.path_high_dose_train = sorted(self.path_high_dose_train)

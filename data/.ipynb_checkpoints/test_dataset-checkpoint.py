...
class TestDataset(BaseDataset):
    ...
    def __init__(self, opt,phase):
        ...
        self.dir_B = os.path.join(opt.dataroot_valid, 'noisy_25')
    ...
    def __getitem__(self, index):
        ...
        #B = np.load(B_path)
        A = np.expand_dims(np.load(A_path,allow_pickle=True),2)
        B = np.expand_dims(np.load(B_path),2)
        A = self.np2Tensor(A)[0]
        B = self.np2Tensor(B)[0]
        return {'hr': A, 'lr': B, 'hr_paths': A_path, 'lr_paths': B_path}
